...
struct DTFileController
{
    ...
    @nonobjc
    public static
    let main: DTFileController = DTFileController()
    ...
    {
        let result = self.fileManager.fileExists(atPath: path)
     ...
     */
    public
    func getFreeSpace() throws -> UInt64
    {
     ...
     */
    public
    func checkSpaceEnough(withFilePath path: String) throws -> Bool
    {
     ...
     */
    public
    func checkSpaceEnough(forFileSize size: UInt64) throws -> Bool
    {
        let freeSpace: UInt64 = try self.getFreeSpace()
     ...
     */
    public
    func currentApplicationPath(withFileName fileName: String = "", ofType fileExtension: String? = nil) -> String?
    {
    ...
    */
    public
    func currentApplicationURL(withFileName fileName: String = "", ofType fileExtension: String? = nil) -> URL?
    {
        let bundle = Bundle.main
     ...
     */
    public
    func documentPath(withFileName fileName: String = "") -> String
    {
     ...
     */
    public
    func documentUrl(withFileName fileName: String = "") -> URL
    {
     ...
     */
    public
    func cachesPath(withFileName fileName: String = "") -> String
    {
     ...
     */
    public
    func cachesUrl(withFileName fileName: String = "") -> URL
    {
     ...
     */
    public
    func sharePath(withFileName fileName: String = "", groupIdentifier: String) -> String?
    {
     ...
     */
    public
    func shareUrl(withFileName fileName: String = "", groupIdentifier: String) -> URL?
    {
        guard let pathURL: URL = self.fileManager.containerURL(forSecurityApplicationGroupIdentifier: groupIdentifier) else {
     ...
     */
    public
    func createDirectory(atPath path: String, withIntermediateDirectories createIntermediates: Bool = false) throws
    {
     ...
     */
    public
    func createDirectory(at url: URL, withIntermediateDirectories createIntermediates: Bool = false) throws
    {
    ...
    @discardableResult
    public
    func createFile(atPath path: String, contents: Data? = nil) -> Bool
    {
     ...
     */
    public
    func isExcludedFromBackup(atPath path: String) throws -> Bool
    {
    ...
    /**
     Get directory or file is avoid backup from given file URL.
     
     - Parameter url: A file path url to check is excluded backup.
     ...
     */
    public
    func isExcludedFromBackup(atUrl url: NSURL) throws -> Bool
    {
        let resourceKey: URLResourceKey = .isExcludedFromBackupKey
        let resources: Dictionary<URLResourceKey, Any> = try url.resourceValues(forKeys: [resourceKey])
     ...
     */
    public
    func setExcludeBackupAttribute(_ excluded: Bool = true, atPath path: String) throws
    {
        let url = NSURL(fileURLWithPath: path)
    ...
    /**
     Set directory or file path of URL object to avoid backup.
     ...
     */
    public
    func setExcludeBackupAttribute(_ excluded: Bool = true, atUrl url: NSURL) throws
    {
        try url.setResourceValue(excluded, forKey: URLResourceKey.isExcludedFromBackupKey)
    }
    ...
    /**
     ...
     - Parameter mask: Options for the enumeration. Because this method performs only shallow enumerations, options that prevent descending into subdirectories or packages are not allowed; the only supported option is [skipsHiddenFiles](https://developer.apple.com/documentation/foundation/filemanager/directoryenumerationoptions/1415528-skipshiddenfiles).
     ...
     */
    public
    func contentsOfDirectory(at url: URL, includingPropertiesForKeys keys: Array<URLResourceKey>?, options mask: FileManager.DirectoryEnumerationOptions = []) throws -> Array<URL>
    {
        let contents: Array<URL> = try self.fileManager.contentsOfDirectory(at: url, includingPropertiesForKeys: keys, options: mask)
     ...
     */
    public
    func contentsOfDirectory(atPath path: String) throws -> Array<String>
    {
     ...
     */
    public
    func copyFile(atPath path: String, toPath: String) throws
    {
     ...
     */
    public
    func copyFile(at srcUrl: URL, to destUrl: URL) throws
    {
        try self.fileManager.copyItem(at: srcUrl, to: destUrl)
    }
    
    /**
     Copy Copy file to destination path, With the process progress.
     
     - Parameter path:      The source file path to copy.
     - Parameter toPath:    The destination path will be copy.
     - Parameter progress:  The closure to know the process progress.
     - Parameter complete:  When process done, the block will respond.
     
     - Throws: Error type object when copy failed.
     */
    public
    func copyFile(atPath path: String, toPath: String, progress: ProgressHandler? = nil, complete: @escaping OperationHandler) throws
    {
        let url = URL(fileURLWithPath: path)
        let toUrl = URL(fileURLWithPath: toPath)
        
        try self.copyFile(at: url, to: toUrl, progress: progress, complete: complete)
    }
    
    /**
     Copy Copy file to destination path, With the process progress.
     
     - Parameter url:       The source file URL to copy.
     - Parameter toUrl:     The destination file URL will be copy.
     - Parameter progress:  The closure to know the process progress.
     - Parameter complete:  When process done, the block will respond.
     ...
     */
    public
    func copyFile(at url: URL, to toUrl: URL, progress: ProgressHandler? = nil, complete: @escaping OperationHandler) throws
    {
        guard url != toUrl else {
            
            throw Error.samePath
        }
        
        if !self.fileExists(atUrl: url) {
            
            throw Error.sourceFileNotExists
        }
        
        var toUrl: URL = toUrl
        
        if try self.isDirectory(withUrl: toUrl) {
            
            let fileName: String = url.lastPathComponent
            toUrl.appendPathComponent(fileName)
        }
        
        if !self.fileExists(atUrl: toUrl) {
            
            self.createFile(at: toUrl)
        }
        
        let sourceFileHandle = try FileHandle(forReadingFrom: url)
        let destinationFileHandle = try FileHandle(forWritingTo: toUrl)
        let fileSize: UInt64 = try self.getFileSize(atPath: url.path)
        
        let copyOperationQueue: @Sendable () -> Void = {
            
            var offset: UInt64 = 0
            let chunkSize: Int = 1024 * 100
            
            repeat {
                
                autoreleasepool {
                    
                    // Seek to next direction.
                    sourceFileHandle.seek(toFileOffset: offset)
                    
                    // Reading a chunk of data from source file.
                    let writeData: Data = sourceFileHandle.readData(ofLength: chunkSize)
                    
                    // Write data from source to destination.
                    destinationFileHandle.write(writeData)
                    
                    // Couting percent
                    if let progress = progress {
                        
                        offset += UInt64(writeData.count)
                        let percent: Float = Float(offset) / Float(fileSize)
        ...
        let operation = BlockOperation(block: copyOperationQueue)
        let operationQueue = OperationQueue()
        operationQueue.name = "Copy Queue"
     ...
     */
    public
    func moveFile(atPath path: String, toPath: String) throws
    {
     ...
     */
    public
    func moveFile(at srcUrl: URL, to destUrl: URL) throws
    {
     ...
     */
    public
    func moveFile(atPath path: String, toPath: String, progress: ProgressHandler? = nil, complete: @escaping OperationHandler) throws
    {
        let url = URL(fileURLWithPath: path)
        let toUrl = URL(fileURLWithPath: toPath)
        
        try self.moveFile(at: url, to: toUrl, progress: progress, complete: complete)
    }
     ...
     */
    public
    func moveFile(at url: URL, to toUrl: URL, progress: ProgressHandler? = nil, complete: @escaping OperationHandler) throws
    {
        let completeHandler: OperationHandler = {
            
            Task.detached {
                
                try? await self.removeFile(at: url)
                complete()
            }
        }
        
        try self.copyFile(at: url, to: toUrl, progress: progress, complete: completeHandler)
    }
     ...
     */
    public
    func attributesOfFileSystem(atPath path: String) throws -> FileAttribute
    {
        let attributes: FileAttribute = try self.fileManager.attributesOfFileSystem(forPath: path)
    ...
    {
        let attributes: FileAttribute = try self.attributesOfItem(atPath: path)
     ...
     */
    public
    func getFileSize(atPath path: String, convertSizeUnit converted: Bool = false) throws -> String
    {
        let attributes: FileAttribute = try self.attributesOfFileSystem(atPath: path)
     ...
     */
    public
    func getFileCreationDate(atPath path: String) throws -> Date?
    {
        let attributes: FileAttribute = try self.attributesOfItem(atPath: path)
     ...
     */
    public
    func getFileCreationDate(atPath path: String, dateFormat: String = "YYYY/MM/dd HH:mm:ss aa") throws -> String?
    {
     ...
     */
    public
    func getFileModificationDate(atPath path: String) throws -> Date?
    {
        let attributes: FileAttribute = try self.attributesOfItem(atPath: path)
     ...
     */
    public
    func getFileModificationDate(atPath path: String, dateFormat: String = "YYYY/MM/dd HH:mm:ss aa") throws -> String?
    {
     ...
     */
    public
    func isDirectory(withUrl url: URL) throws -> Bool
    {
        guard url.isFileURL else {
            return false
        }
...
public 
extension DTFileController
{
    typealias FileAttribute = Dictionary<FileAttributeKey, Any>
    
    typealias ProgressHandler = @Sendable (Float) -> Void
    typealias OperationHandler = @Sendable () -> Void
    
    enum Error: Swift.Error
    {
        case samePath
        case sourceFileNotExists
        case readingFileInformationFailed
    }
}

extension DTFileController.Error: CustomStringConvertible
{
    public
    var description: String {
        ...
        switch self {
        case .samePath:
            description = "Sorce and destination path is same one."
            
        case .sourceFileNotExists:
            description = "Sorce file not exists."
            
        case .readingFileInformationFailed:
            description = "Failed to reading file inforamtion."
        }
...
extension DTFileController
{
    func getFreeSpace(atPath path: String, converSizeUnit convered: Bool = false) throws -> String
    {
        let fileSystemInfomation: FileAttribute = try self.fileManager.attributesOfFileSystem(forPath: path)
    ...
    func getFreeSpace(atPath path: String) throws -> UInt64
    {
        let fileSystemInfomation: FileAttribute = try self.fileManager.attributesOfFileSystem(forPath: path)
    ...
    func attributesOfItem(atPath path: String) throws -> FileAttribute
    {
        let attributes: FileAttribute = try self.fileManager.attributesOfItem(atPath: path)
...
{
    static
    func convertFileSize(_ size: Int64) -> String
    {
